...
int main(int argc, char *argv[])
{
    ...
    cmd.AddValue("trustThreshold", "Trust threshold (0.0-1.0, default 0.5)",   trustThreshold);
    cmd.Parse(argc, argv);
    ...
    if (attack != "none" && nMalicious > 0)
    {
        ...
        for (uint32_t i = 0; i < nMalicious && i < nNodes - 2; i++)
        {
            ...
            // Make sure it's not a source (0..maxConn-1) or dest (nNodes-maxConn..nNodes-1)
    ...
    if (defense == "ids") {
        ...
        NS_LOG_UNCOND("** Defense: Trust-Based AODV (neighbor monitor, threshold=" << trustThreshold << ") **");
    }
    ...
    for (uint32_t i = 0; i < maxConn; i++) {
        ...
        onoff.SetConstantRate(DataRate("10240bps"));   // Paper: 10 kb CBR
