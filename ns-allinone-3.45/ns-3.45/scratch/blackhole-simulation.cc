...
int main(int argc, char *argv[])
{
    ...
    std::cout << " Seed        : " << seed       << "\n";
    std::cout << " Area        : 1000 x 1500 m\n";
    std::cout << " WiFi Range  : 350 m\n";
    ...
    double   areaX     = 800.0;   // Paper: 1000m
    double   areaY     = 800.0;   // Paper: 1500m
    double   gridDX    = areaX / (gridWidth + 1);
    double   gridDY    = areaY / (gridWidth + 1);
    ...
    for (uint32_t i = 0; i < maxConn; i++) {
        ...
        srcApp.Start(Seconds(8.0));    // let AODV routes converge
    ...
    for (auto& flow : stats) {
        ...
        // Skip AODV control packets (port 654)
