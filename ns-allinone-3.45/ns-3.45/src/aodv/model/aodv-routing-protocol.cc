...
RoutingProtocol::RoutingProtocol()
    : m_rreqRetries(2),
      ...
      m_enableHello(false),
      m_isMalicious(false),
      m_maliciousSeqNoBoost(1000),
      m_isGrayhole(false),
      m_grayholeDropProb(0.5),
      m_isSmartBlackhole(false),
      m_isSmartGrayhole(false),
      m_enableIDS(false),
      m_enableMBDP(false),
      m_mbdpK(4),
      m_enableTrust(false),
      m_trustThreshold(0.5),
      m_ratingWeight(0.3),
    ...
    static TypeId tid =
        TypeId("ns3::aodv::RoutingProtocol")
            ...
            .AddAttribute("IsMalicious",
                          "Indicates whether this node behaves as a Black Hole attacker.",
                          BooleanValue(false),
                          MakeBooleanAccessor(&RoutingProtocol::SetIsMalicious,
                                              &RoutingProtocol::GetIsMalicious),
                          MakeBooleanChecker())
            .AddAttribute("MaliciousSeqNoBoost",
                          "Sequence number boost for malicious RREP (Black Hole attack).",
                          UintegerValue(1000),
                          MakeUintegerAccessor(&RoutingProtocol::SetMaliciousSeqNoBoost,
                                               &RoutingProtocol::GetMaliciousSeqNoBoost),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("IsGrayhole",
                          "Indicates whether this node behaves as a Gray Hole attacker.",
                          BooleanValue(false),
                          MakeBooleanAccessor(&RoutingProtocol::SetIsGrayhole,
                                              &RoutingProtocol::GetIsGrayhole),
                          MakeBooleanChecker())
            .AddAttribute("GrayholeDropProbability",
                          "Probability of dropping a packet in Gray Hole mode (0.0-1.0).",
                          DoubleValue(0.5),
                          MakeDoubleAccessor(&RoutingProtocol::SetGrayholeDropProbability,
                                             &RoutingProtocol::GetGrayholeDropProbability),
                          MakeDoubleChecker<double>(0.0, 1.0))
            .AddAttribute("IsSmartBlackhole",
                          "Smart Black Hole: no fake RREP, drops all data packets.",
                          BooleanValue(false),
                          MakeBooleanAccessor(&RoutingProtocol::SetIsSmartBlackhole,
                                              &RoutingProtocol::GetIsSmartBlackhole),
                          MakeBooleanChecker())
            .AddAttribute("IsSmartGrayhole",
                          "Smart Gray Hole: no fake RREP, drops selective data packets.",
                          BooleanValue(false),
                          MakeBooleanAccessor(&RoutingProtocol::SetIsSmartGrayhole,
                                              &RoutingProtocol::GetIsSmartGrayhole),
                          MakeBooleanChecker())
            .AddAttribute(
                "EnableIDS",
                "IDS-AODV: ignore first RREP per destination to mitigate black-hole.",
                BooleanValue(false),
                MakeBooleanAccessor(&RoutingProtocol::SetEnableIDS, &RoutingProtocol::GetEnableIDS),
                MakeBooleanChecker())
            .AddAttribute("EnableMBDP",
                          "MBDP-AODV: mean-based detection and prevention of black-hole.",
                          BooleanValue(false),
                          MakeBooleanAccessor(&RoutingProtocol::SetEnableMBDP,
                                              &RoutingProtocol::GetEnableMBDP),
                          MakeBooleanChecker())
            .AddAttribute(
                "MbdpK",
                "MBDP-AODV: number of RREP replies to collect before statistical analysis.",
                UintegerValue(4),
                MakeUintegerAccessor(&RoutingProtocol::SetMbdpK, &RoutingProtocol::GetMbdpK),
                MakeUintegerChecker<uint32_t>())
            .AddAttribute(
                "EnableTrust",
                "Trust-Based AODV: monitors neighbor forwarding behavior, avoids unreliable nodes.",
                BooleanValue(false),
                MakeBooleanAccessor(&RoutingProtocol::SetEnableTrust,
                                    &RoutingProtocol::GetEnableTrust),
                MakeBooleanChecker())
            .AddAttribute("TrustThreshold",
                          "Trust-Based AODV: threshold below which a neighbor is untrusted.",
                          DoubleValue(0.5),
                          MakeDoubleAccessor(&RoutingProtocol::SetTrustThreshold,
                                             &RoutingProtocol::GetTrustThreshold),
                          MakeDoubleChecker<double>(0.0, 1.0))
                             ...
                             Socket::SocketErrno& sockerr)
{
    ...
    // neighbour k monitor kore in trust algo
    if (m_enableTrust)
    {
        Time now = Simulator::Now();
        for (auto it = m_pendingForwards.begin(); it != m_pendingForwards.end();)
        {
            if (it->expiry < now)
            {
                RefreshNeighborRating(it->nextHop);
                it = m_pendingForwards.erase(it);
            }
            else
            {
                ++it;
    ...
    if (m_routingTable.LookupValidRoute(dst, rt))
    {
        ...
        if (m_enableTrust)
        {
            ...
            if (nextHop != dst)
            {
                double trust = (m_neighborRating.count(nextHop)) ? m_neighborRating[nextHop] : 1.0;
                if (trust < m_trustThreshold)
                {
                    NS_LOG_DEBUG("NBRMON: Source avoiding untrusted next hop "
                                 << nextHop << " trust=" << trust);
                ...
                PendingForwardEntry entry;
                entry.origin = header.GetSource();
                entry.dst = dst;
                entry.identification = header.GetIdentification();
                entry.nextHop = nextHop;
                entry.expiry = Simulator::Now() + Seconds(1.0);
                m_pendingForwards.push_back(entry);
                m_sentCount[nextHop]++;
            }
        }
                            ...
                            ErrorCallback ecb)
{
    ...
    // BLACK HOLE ATTACK
    if (m_isMalicious || m_isSmartBlackhole)
    {
        NS_LOG_DEBUG("MALICIOUS NODE: Dropping packet from "
                     << origin << " to " << dst << " (packet UID: " << p->GetUid() << ")");
        return true;
    }
    ...
    if (m_isGrayhole)
    {
        double randVal = m_uniformRandomVariable->GetValue(0.0, 1.0);
        if (randVal < m_grayholeDropProb)
        {
            NS_LOG_DEBUG("GRAYHOLE NODE: Dropping packet from " << origin << " to " << dst
                                                                << " (prob=" << m_grayholeDropProb
                                                                << ", rand=" << randVal << ")");
            return true;
        }
        NS_LOG_DEBUG("GRAYHOLE NODE: Forwarding packet from " << origin << " to " << dst
                                                              << " (prob=" << m_grayholeDropProb
                                                              << ", rand=" << randVal << ")");
    }

    // SMART GRAY HOLE ATTACK
    if (m_isSmartGrayhole)
    {
        double randVal = m_uniformRandomVariable->GetValue(0.0, 1.0);
        if (randVal < m_grayholeDropProb)
        {
            NS_LOG_DEBUG("SMART GRAYHOLE: Dropping packet from " << origin << " to " << dst
                                                                 << " (prob=" << m_grayholeDropProb
                                                                 << ", rand=" << randVal << ")");
            return true;
        }
        NS_LOG_DEBUG("SMART GRAYHOLE: Forwarding packet from " << origin << " to " << dst
                                                               << " (prob=" << m_grayholeDropProb
                                                               << ", rand=" << randVal << ")");
    }
    ...
    if (m_enableTrust)
    {
        ...
        for (auto it = m_pendingForwards.begin(); it != m_pendingForwards.end();)
        {
            if (it->expiry < now)
            {
                // Expired — neighbor probably didn't forward
                // pktsSentVia was already incremented when added to buffer
                RefreshNeighborRating(it->nextHop);
                it = m_pendingForwards.erase(it);
            }
            else
    ...
    if (m_routingTable.LookupRoute(dst, toDst))
    {
        if (toDst.GetFlag() == VALID)
        {
            ...
            if (m_enableTrust)
            {
                Ipv4Address nextHop = toDst.GetNextHop();
                double trust = (m_neighborRating.count(nextHop)) ? m_neighborRating[nextHop] : 1.0;
                if (trust < m_trustThreshold)
                {
                    NS_LOG_DEBUG("NBRMON: Untrusted next hop "
                                 << nextHop << " trust=" << trust << " < " << m_trustThreshold
                                 << ". Deleting route, triggering re-discovery.");
                    m_routingTable.DeleteRoute(dst);
            ...
            if (m_enableTrust)
            {
                ...
                if (nextHop != dst)
                {
                    ...
                    m_pendingForwards.push_back(entry);
                    m_sentCount[nextHop]++;
                    NS_LOG_DEBUG("NBRMON: Watching packet {" << origin << "->" << dst
...
bool
RoutingProtocol::OnOverheardPacket(Ptr<NetDevice> dev,
                                   Ptr<const Packet> pkt,
                                   uint16_t protocol,
                                   const Address& from,
                                   const Address& to,
                                   NetDevice::PacketType pktType)
{
    if (!m_enableTrust)
    {
        return true;
    ...
    if (pktType == NetDevice::PACKET_HOST)
    {
        return true;
    }
    ...
    // Skip AODV control packets (UDP port 654)
    if (ipHeader.GetProtocol() == 17)
    {
        UdpHeader udpHeader;
        if (copy->GetSize() >= 8 && copy->PeekHeader(udpHeader) > 0)
        {
    ...
    Ipv4Address overhearOrigin = ipHeader.GetSource();
    Ipv4Address overhearDst = ipHeader.GetDestination();
    uint16_t overhearId = ipHeader.GetIdentification();

    for (auto it = m_pendingForwards.begin(); it != m_pendingForwards.end(); ++it)
    {
        if (it->origin == overhearOrigin && it->dst == overhearDst &&
            it->identification == overhearId)
        {
            // Neighbor forwarded the packet!
            Ipv4Address nextHop = it->nextHop;
            m_heardCount[nextHop]++;
            NS_LOG_DEBUG("NBRMON: Overheard forwarding by " << nextHop << " {" << overhearOrigin
                                                            << "->" << overhearDst
                                                            << " id=" << overhearId << "}");

            // Remove from buffer (confirmed forwarded)
            m_pendingForwards.erase(it);
            RefreshNeighborRating(nextHop);
            break;
        }
    }
    return true;
...
void
RoutingProtocol::RefreshNeighborRating(Ipv4Address neighbor)
{
    double sent = (m_sentCount.count(neighbor)) ? (double)m_sentCount[neighbor] : 0.0;
    double heard = (m_heardCount.count(neighbor)) ? (double)m_heardCount[neighbor] : 0.0;
    ...
    double ratio = heard / sent;
    double oldTrust = (m_neighborRating.count(neighbor)) ? m_neighborRating[neighbor] : 1.0;
    double newTrust = m_ratingWeight * ratio + (1.0 - m_ratingWeight) * oldTrust;

    m_neighborRating[neighbor] = newTrust;

    NS_LOG_DEBUG("NBRMON: Updated " << neighbor << " sent=" << sent << " heard=" << heard
                                    << " ratio=" << ratio << " trust=" << newTrust);

    if (newTrust < m_trustThreshold)
    {
        NS_LOG_DEBUG("NBRMON: *** Node " << neighbor << " is now UNTRUSTED! ***");
    }
}
...
RoutingProtocol::NotifyInterfaceUp(uint32_t i)
{
    ...
    if (m_enableTrust)
    {
        Ptr<NetDevice> promiscDev = l3->GetNetDevice(i);
        promiscDev->SetPromiscReceiveCallback(
            MakeCallback(&RoutingProtocol::OnOverheardPacket, this));
        NS_LOG_DEBUG("NBRMON: Promiscuous mode enabled on interface " << i);
    }
...
RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    ...
    if (m_isMalicious && !IsMyOwnAddress(rreqHeader.GetDst()))
    {

        NS_LOG_DEBUG("MALICIOUS NODE: Sending fake RREP for destination " << rreqHeader.GetDst());

        Ptr<NetDevice> dev = m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress(receiver));

        uint32_t fakeSeqNo = rreqHeader.GetDstSeqno() + m_maliciousSeqNoBoost;

        RoutingTableEntry fakeEntry(
            /*dev=*/dev,
            /*dst=*/rreqHeader.GetDst(),
            /*vSeqNo=*/true,
            /*seqNo=*/fakeSeqNo,
            /*iface=*/m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(receiver), 0),
            /*hops=*/1,           // Claim to  very close
            /*nextHop=*/receiver, // Next hop is myself
        ...
        RrepHeader rrepHeader(
            /*prefixSize=*/0,
            /*hopCount=*/1, // Claim 1 hop to destination
            /*dst=*/rreqHeader.GetDst(),
            /*dstSeqNo=*/fakeSeqNo,
            /*origin=*/origin,
            /*lifeTime=*/m_myRouteTimeout);
        ...
        NS_LOG_DEBUG("MALICIOUS NODE: Sent fake RREP with SeqNo=" << fakeSeqNo << " HopCount=1");
        return;
    }

    
    if (m_isGrayhole && !IsMyOwnAddress(rreqHeader.GetDst()))
    {
        NS_LOG_DEBUG("GRAYHOLE NODE: Sending fake RREP for destination " << rreqHeader.GetDst());
        ...
        NS_LOG_DEBUG("GRAYHOLE NODE: Sent fake RREP with SeqNo=" << fakeSeqNo << " DropProb="
                                                                 << m_grayholeDropProb);
        return;
    }
...
RoutingProtocol::RecvReply(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
    ...
    if (m_enableIDS && IsMyOwnAddress(rrepHeader.GetOrigin()))
    {
        if (m_rrepCount.find(dst) == m_rrepCount.end())
        {
            // First RREP for this destination — ignore it
            m_rrepCount[dst] = 1;
            NS_LOG_DEBUG("IDS-AODV: Ignoring first RREP for dst="
                         << dst << " SeqNo=" << rrepHeader.GetDstSeqno()
                         << " HopCount=" << static_cast<uint32_t>(hop));
            return;
        }
        else
        {
            m_rrepCount[dst]++;
            NS_LOG_DEBUG("IDS-AODV: Accepting RREP #"
                         << m_rrepCount[dst] << " for dst=" << dst << " SeqNo="
                         << rrepHeader.GetDstSeqno() << " HopCount=" << static_cast<uint32_t>(hop));
        }
    }
    ...
    if (m_enableMBDP)
    {
        if (m_mbdpBlacklist.find(sender) != m_mbdpBlacklist.end())
        {
            NS_LOG_DEBUG("MBDP-AODV: Dropping RREP from blacklisted node " << sender);
            return;
        }

        if (m_mbdpSuspectedSeqNo.find(rrepHeader.GetDstSeqno()) != m_mbdpSuspectedSeqNo.end())
        {
            NS_LOG_DEBUG(
                "MBDP-AODV: Dropping RREP with suspected SeqNo=" << rrepHeader.GetDstSeqno());
            return;
        }
    }
    ...
    if (IsMyOwnAddress(rrepHeader.GetOrigin()))
    {
        ...
        if (m_enableMBDP)
        {
            m_mbdpSeqNoTable[dst].push_back(rrepHeader.GetDstSeqno());
            m_mbdpRrepCount[dst]++;

            NS_LOG_DEBUG("MBDP-AODV: Collected RREP #"
                         << m_mbdpRrepCount[dst] << "/" << m_mbdpK << " for dst=" << dst
                         << " SeqNo=" << rrepHeader.GetDstSeqno()
                         << " HopCount=" << static_cast<uint32_t>(hop));

           
            if (m_mbdpRrepCount[dst] >= m_mbdpK)
            {
                std::vector<uint32_t>& seqNos = m_mbdpSeqNoTable[dst];
                uint32_t K = seqNos.size();

                //mean r deviation calculate
                double sum = 0.0;
                for (uint32_t i = 0; i < K; i++)
                {
                    sum += static_cast<double>(seqNos[i]);
                }
                ...
                for (uint32_t i = 0; i < K; i++)
                {
                    double diff = static_cast<double>(seqNos[i]) - average;
                    variance += diff * diff;
                }
                double threshold = std::sqrt(variance / K);

                NS_LOG_DEBUG("MBDP-AODV: Analysis for dst=" << dst << " K=" << K
                                                            << " Mean=" << average
                                                            << " StdDev(Threshold)=" << threshold);

                
                if (threshold > average)
                {
                    NS_LOG_DEBUG("MBDP-AODV: Suspicious activity detected! "
                                 << "Threshold(" << threshold << ") > Average(" << average << ")");

                    
                    RoutingTableEntry checkEntry;
                    if (m_routingTable.LookupRoute(dst, checkEntry))
                    {
                        uint32_t rtSeqNo = checkEntry.GetSeqNo();
                        uint32_t rtHops = checkEntry.GetHop();
                        Ipv4Address nextHop = checkEntry.GetNextHop();

                        if (static_cast<double>(rtSeqNo) > threshold && rtHops == 1)
                        {
                            m_mbdpBlacklist.insert(nextHop);
                            m_mbdpSuspectedSeqNo.insert(rtSeqNo);

                            NS_LOG_DEBUG("MBDP-AODV: BLACKLISTED node "
                                         << nextHop << " (SeqNo=" << rtSeqNo << " Hops=" << rtHops
                                         << ")");
                            ...
                            NS_LOG_DEBUG("MBDP-AODV: Deleted malicious route to "
                                         << dst << ", will re-discover route");
                        }
                        else if (static_cast<double>(rtSeqNo) > threshold && rtHops != 1)
                        {
                            m_mbdpSuspectedSeqNo.insert(rtSeqNo);
                            NS_LOG_DEBUG("MBDP-AODV: Suspected node via "
                                         << nextHop << " (SeqNo=" << rtSeqNo << " Hops=" << rtHops
                                         << ") — forwarding SUSPECT");
                            m_routingTable.DeleteRoute(dst);
                        }
                ...
                {
                    NS_LOG_DEBUG("MBDP-AODV: No suspicious activity. "
                                 << "Threshold(" << threshold << ") <= Average(" << average << ")");
                }

                m_mbdpSeqNoTable.erase(dst);
                m_mbdpRrepCount.erase(dst);
            }
            else
            {
                NS_LOG_DEBUG("MBDP-AODV: Waiting for more RREPs (" << m_mbdpRrepCount[dst] << "/"
                                                                   << m_mbdpK << ")");
                return;
            }
        }
