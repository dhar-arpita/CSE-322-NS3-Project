...
class RoutingProtocol : public Ipv4RoutingProtocol
{
  public:
    ...
    bool GetBroadcastEnable() const
    {
     ...
     // Set flag for blackhole
    
    void SetIsMalicious(bool f)
    {
        m_isMalicious = f;
    }

    
    //Get flag for blackhole
     
    bool GetIsMalicious() const
    {
        return m_isMalicious;
    }

    // set sequence num for blackhole
    void SetMaliciousSeqNoBoost(uint32_t boost)
    {
        m_maliciousSeqNoBoost = boost;
    }

    
    // Get sequence number for blackhole
     
    uint32_t GetMaliciousSeqNoBoost() const
    {
        return m_maliciousSeqNoBoost;
    }

    // set flag for grayhole
    void SetIsGrayhole(bool f)
    {
        m_isGrayhole = f;
    }

    //get flag for grayhole
    bool GetIsGrayhole() const
    {
        return m_isGrayhole;
    }

    //set drop probability
    void SetGrayholeDropProbability(double p)
    {
        m_grayholeDropProb = p;
    }

   //get drop probability
    double GetGrayholeDropProbability() const
    {
        return m_grayholeDropProb;
    }

    //for smart blackhole
    void SetIsSmartBlackhole(bool f)
    {
        m_isSmartBlackhole = f;
    }

    bool GetIsSmartBlackhole() const
    {
        return m_isSmartBlackhole;
    }

    //for smart grayhole
    void SetIsSmartGrayhole(bool f)
    {
        m_isSmartGrayhole = f;
    }

    bool GetIsSmartGrayhole() const
    {
        return m_isSmartGrayhole;
    }

    /**
     * Set IDS-AODV enable flag (ignores first RREP, accepts subsequent)
     * @param f the IDS enable flag
     */
    void SetEnableIDS(bool f)
    {
        m_enableIDS = f;
    }

    /**
     * Get IDS-AODV enable flag
     * @returns the IDS enable flag
     */
    bool GetEnableIDS() const
    {
        return m_enableIDS;
    }

    /**
     * Set MBDP-AODV enable flag (mean-based detection and prevention)
     * @param f the MBDP enable flag
     */
    void SetEnableMBDP(bool f)
    {
        m_enableMBDP = f;
    }

    /**
     * Get MBDP-AODV enable flag
     * @returns the MBDP enable flag
     */
    bool GetEnableMBDP() const
    {
        return m_enableMBDP;
    }

    /**
     * Set MBDP-AODV K value (number of RREPs to collect before analysis)
     * @param k the number of replies to collect
     */
    void SetMbdpK(uint32_t k)
    {
        m_mbdpK = k;
    }

    /**
     * Get MBDP-AODV K value
     * @returns the K value
     */
    uint32_t GetMbdpK() const
    {
        return m_mbdpK;
    }

    ///@name Trust-Based AODV (Neighbor Forwarding Monitor)
    ///@{
    void SetEnableTrust(bool f)
    {
        m_enableTrust = f;
    }

    bool GetEnableTrust() const
    {
        return m_enableTrust;
    }

    void SetTrustThreshold(double t)
    {
        m_trustThreshold = t;
    }

    double GetTrustThreshold() const
    {
        return m_trustThreshold;
    }
    ///@}
  ...
  private:
    ...
    bool m_isMalicious;      //for malicious flag
    uint32_t m_maliciousSeqNoBoost; //high sequence number
    bool m_isGrayhole;       ///for grayhole flag
    double m_grayholeDropProb; // dropping probability

    bool m_isSmartBlackhole; //blackhole ->packet paile fele dey but fakerequest pathay na
    bool m_isSmartGrayhole;  //grayhole -> packet paile rkta percentage  fele dey but fakerequest pathay na

    //for IDS 

    bool m_enableIDS;     
    std::map<Ipv4Address, uint32_t> m_rrepCount; 
    ...
    std::map<Ipv4Address, std::vector<uint32_t>> m_mbdpSeqNoTable; 
    std::map<Ipv4Address, uint32_t> m_mbdpRrepCount; 
    std::set<Ipv4Address> m_mbdpBlacklist;  
    std::set<uint32_t> m_mbdpSuspectedSeqNo; 

    // for trust
    bool m_enableTrust;          
    double m_trustThreshold;     
    double m_ratingWeight;        

    //track kore je pcket jetaa pataise oita asolei relay korlo kina
    ...
    std::vector<PendingForwardEntry> m_pendingForwards;  
    std::map<Ipv4Address, double> m_neighborRating;   
    std::map<Ipv4Address, uint32_t> m_sentCount;   
    std::map<Ipv4Address, uint32_t> m_heardCount;  

   
    bool OnOverheardPacket(Ptr<NetDevice> dev, Ptr<const Packet> pkt, uint16_t protocol,
                        const Address& from, const Address& to, NetDevice::PacketType pktType);

    
    void RefreshNeighborRating(Ipv4Address neighbor);
